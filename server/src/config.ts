...
const schema = z.object({
  ...
  FRONTEND_URL: z.url().refine(value => { const url = new URL(value); return ['http:', 'https:'].includes(url.protocol) && url.pathname === '/' && !url.search && !url.hash; }, 'Use one frontend origin without a path'),
